        ...
        if ($data) {
            ...
            if (isset($data['attribute_code'])) {
                ...
                if (!$validatorAttrCode->isValid($data['attribute_code'])) {
                    $session->addError(
                        Mage::helper('catalogattribute')->__('Attribute code is invalid. Please use only letters (a-z), numbers (0-9) or underscore(_) in this field, first character should be a letter.')
                    );
            ...
            if ($id) {
                ...
                if ($model->getEntityTypeId() != $this->_entityTypeId) {
                    $session->addError(
                        Mage::helper('catalogattribute')->__('This attribute cannot be updated.'));
...
//            }
            ...
            try {
                $model->save();
                $session->addSuccess(
                    Mage::helper('catalogattribute')->__('The category attribute has been saved.'));
                ...
                if ($this->getRequest()->getParam('popup')) {
                    $this->_redirect('adminhtml/catalog_category_atribute/addAttribute', array(
